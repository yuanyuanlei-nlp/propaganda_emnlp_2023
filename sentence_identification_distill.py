...
lambda_news_response_distill = 0.5
lambda_news_relation_distill = 0.5
...
class Model(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, label_sentence, news_discourse_prob, pdtb_relation_prob, news_discourse_similarity, pdtb_relation_similarity):
...
def evaluate(model, eval_dataloader, verbose):
    ...
    for step, batch in enumerate(eval_dataloader):
        ...
        input_ids, attention_mask, label_sentence, news_discourse_prob, pdtb_relation_prob, news_discourse_similarity, pdtb_relation_similarity = \
            input_ids.to(device), attention_mask.to(device), label_sentence.to(device), \
        ...
        with torch.no_grad():
            ...
            pdtb_relation_response_distill_loss, pdtb_relation_relation_distill_loss = \
                model(input_ids, attention_mask, label_sentence, news_discourse_prob, pdtb_relation_prob, news_discourse_similarity, pdtb_relation_similarity)
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        if step % ((len(train_dataloader) * num_epochs) // check_times) == 0:
            ...
            if macro_F > best_dev_macro_F:
                torch.save(model.state_dict(), "./saved_models/sentence_identification_distill_best_dev_macro_F.ckpt")
                best_dev_macro_F = macro_F
            if positive_F > best_dev_positive_F:
                torch.save(model.state_dict(), "./saved_models/sentence_identification_distill_best_dev_positive_F.ckpt")
...
model.load_state_dict(torch.load("./saved_models/sentence_identification_distill_best_dev_macro_F.ckpt", map_location=device))
model.eval()
...
model.load_state_dict(torch.load("./saved_models/sentence_identification_distill_best_dev_positive_F.ckpt", map_location=device))
model.eval()
macro_F, positive_F = evaluate(model, test_dataloader, verbose = 1)
