    ...
    def __getitem__(self, idx):
        ...
        for sent_i in range(len(article_json['sentences'])):
            ...
            if start + len(sentence_encoding['input_ids']) < MAX_LEN:  # truncate to MAX_LEN
                ...
                input_ids.extend(tokenizer.encode_plus(sentence_text + '</s>', add_special_tokens=False)['input_ids'])
                attention_mask.extend(tokenizer.encode_plus(sentence_text + '</s>', add_special_tokens=False)['attention_mask'])
...
class Model(nn.Module):
    ...
    def __init__(self):
        ...
        self.propaganda_1 = nn.Linear(768 + 9 + 8, 768, bias=True)
    ...
    def forward(self, input_ids, attention_mask, label_sentence, news_discourse_prob, pdtb_relation_prob):
        ...
        sentence_embedding = torch.cat((sentence_embedding, news_discourse_prob), dim = 1)

        # additional pdtb relation feature
        torch_zeros = torch.zeros(1, 4).to(device)
        latter_pdtb = torch.cat((pdtb_relation_prob, torch_zeros), dim = 0)
        previours_pdtb = torch.cat((torch_zeros, pdtb_relation_prob), dim = 0)
        sentence_embedding = torch.cat((sentence_embedding, torch.cat((latter_pdtb, previours_pdtb), dim = 1)), dim = 1)

        # propaganda task
        propaganda_scores = self.propaganda_2(self.relu(self.propaganda_1(sentence_embedding)))
        propaganda_loss = self.crossentropyloss(propaganda_scores, label_sentence[:, 2])
...
def evaluate(model, eval_dataloader, verbose):
    ...
    for step, batch in enumerate(eval_dataloader):
        ...
        input_ids, attention_mask, label_sentence, news_discourse_prob, pdtb_relation_prob = \
            input_ids.to(device), attention_mask.to(device), label_sentence.to(device), news_discourse_prob.to(device), pdtb_relation_prob.to(device)

        with torch.no_grad():
            propaganda_scores, propaganda_loss = \
                model(input_ids, attention_mask, label_sentence, news_discourse_prob, pdtb_relation_prob)

        decision = torch.argmax(propaganda_scores, dim = 1).view(propaganda_scores.shape[0], 1)
        true_label = label_sentence[:, 2].view(propaganda_scores.shape[0], 1)
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        if step % ((len(train_dataloader) * num_epochs) // check_times) == 0:
            ...
            if macro_F > best_dev_macro_F:
                torch.save(model.state_dict(), "./saved_models/sentence_identification_additional_best_dev_macro_F.ckpt")
                best_dev_macro_F = macro_F
            if positive_F > best_dev_positive_F:
                torch.save(model.state_dict(), "./saved_models/sentence_identification_additional_best_dev_positive_F.ckpt")
        ...
        propaganda_scores, propaganda_loss = model(input_ids, attention_mask, label_sentence, news_discourse_prob, pdtb_relation_prob)
...
model.load_state_dict(torch.load("./saved_models/sentence_identification_additional_best_dev_macro_F.ckpt", map_location=device))
model.eval()
...
model.load_state_dict(torch.load("./saved_models/sentence_identification_additional_best_dev_positive_F.ckpt", map_location=device))
model.eval()
macro_F, positive_F = evaluate(model, test_dataloader, verbose = 1)
